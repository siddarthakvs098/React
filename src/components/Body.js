...
const Body = () => {
  ...
  const { listOfRestuarant, filteredRestuarant, setFilteredRestuarants } =
    useBody();
  const [searchText, setSearchText , userName] = useState("");
    ...
    <div className="body">
      <div className="flex">
        <div className="search m-4 p-4 ">
          ...
          />
          <button
            className="px-4 py-2 m-4 rounded-lg bg-green-100"
            onClick={(e, restArray) => {
              const filtered = listOfRestuarant.filter((rest) =>
          ...
          <button
            className="m-4 bg-gray-100 px-4 py-2   rounded-lg "
            onClick={(params) => {
              setFilteredRestuarants(
                listOfRestuarant.filter((rest) => rest.info.avgRating >= 4)
              );
            }}
          >
      ...
      <div className="flex flex-wrap">
        ...
        {filteredRestuarant.map((rest) => (
          <Link key={rest.info.id} to={"/restuarant/" + rest.info.id}>
